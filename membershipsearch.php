<html>

<link rel="stylesheet" type="text/css" href="CCBStyleSheet.css">

<head>
    <meta charset="UTF-8">
    <title></title>
</head>

<ul>
    <li><a href="home.html">Home</a></li>
    <li><a  href="playerRegistration.php">Registration</a></li>
    <li><a class="active" href="playerSearch.php">Information</a></li>
</ul>

<div class="MemberProfile">
    <h1 class="pageName">Member Details</h1>
</div>

<ul class="subMenu">
    <li><a href="playerRegistration.php">Player Profile</a></li>
    <li><a href="clubSearch.php">Club Profile</a></li>
    <li><a class="active" href="membershipsearch.php">Membership Details</a></li>
    <li><a href="matchsearch.php">Match Details</a></li>
    <li><a href="home.html">Logout</a></li>
</ul>

<body>

<div class="forms">
    <form class="forms" action="membershipprofile.php" method="post">

        <h4 class="headers">Search using member ID </h4>

        <input type="search" name="memberSearch" title="Search using member ID" placeholder="Search..." required><br><br>

        <input type="submit" name="Search">

    </form>
</div>

</body>

</html>
